...
object ToolsRDD extends java.io.Serializable {
  ...
  def RDDToListByCol(RDD: RDD[(Int, Array[DenseVector[Double]])]): List[List[DenseVector[Double]]]= {
    val RDDAsList = RDD.collect().toList
    RDDAsList.sortBy(_._1).map(_._2.toList).transpose
  }

  def RDDToMatrix(RDD: RDD[(Int, Array[DenseVector[Double]])]): DenseMatrix[DenseVector[Double]] = {
    val RDDAsList = RDD.collect().toList
    val n = RDDAsList.map(_._1).max + 1
    val p = RDDAsList.head._2.length

    val Matrix = DenseMatrix.zeros[DenseVector[Double]](n, p)
    RDDAsList.foreach(e => {
      e._2.indices.foreach(j => {
        Matrix.update(e._1, j, e._2(j))
      })
    })
    Matrix
  }
